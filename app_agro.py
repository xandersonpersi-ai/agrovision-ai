...
st.set_page_config(page_title="AgroVision Pro | Intelligence Híbrida", layout="wide")
...
st.markdown(f"""
    ...
    .source-tag {
        font-size: 12px; padding: 4px 8px; border-radius: 5px; font-weight: bold;
    }
...
# 4. FUNÇÃO GPS HÍBRIDA (MELHORADA)
...
# 6. MODO ANALISAR FOTOS (HÍBRIDO: DRONE + CELULAR)
else:
    uploaded_files = st.file_uploader("📂 ARRASTE AS FOTOS PARA VARREDURA (Drone ou Celular)", accept_multiple_files=True, type=['jpg', 'jpeg', 'png'])
    ...
    if uploaded_files:
        ...
        if dados_lavoura:
            ...
            with col_mapa:
                st.subheader("📍 Georreferenciamento (Fotos de Drone)")
                # Filtra apenas quem tem GPS para não dar bug no mapa
                ...
                if not df_geo.empty:
                    ...
                    st.warning("⚠️ Nenhuma coordenada GPS encontrada. As fotos podem ser de celular sem localização ativa.")
            ...
            with col_intel:
                ...
                st.info(f"**Fonte de dados:** {df['Fonte'].value_counts().to_dict()}")
            ...
            with rec_col2:
                texto_laudo = (
                    ...
                    f"A análise híbrida no talhão **{talhao_id}** indica média de **{media_ponto:.1f}** pragas por ponto. "
                )
                if status_sanitario == "CRÍTICO":
                    texto_laudo += "⚠️ **Ação:** Intervenção recomendada."
                else:
                    texto_laudo += "👍 **Ação:** Níveis sob controle."
                st.info(texto_laudo)
            ...
            for _, row in df.iterrows():
                ...
                with g2:
                    ...
                    <div style="background: white; padding: 20px; border-radius: 15px; border: 1px solid #eee; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                        ...
                        <hr>
                        {"<a href='"+row['Maps_Link']+"' target='_blank'><button class='loc-btn'>📍 LOCALIZAR AGORA</button></a>" if row['Latitude'] else "<i>GPS indisponível nesta amostra</i>"}
                    </div>
                    """, unsafe_allow_html=True)
                st.markdown("---")
