...
st.markdown("""
    ...
    .loc-btn {
        display: inline-block; padding: 15px 25px; font-size: 16px; cursor: pointer; text-align: center; text-decoration: none; 
        color: #fff; background-color: #68CAED; border: 3px solid #FF0000; border-radius: 12px; font-weight: bold; width: 100%;
        animation: neonPulseRed 1.5s infinite ease-in-out; text-transform: uppercase; letter-spacing: 1px;
    }
...
if modo_operacao == "🛸 Drone Real-Time":
    ...
    st.info("As análises de fotos carregadas continuam salvas no relatório abaixo.")
        ...
        for i, file in enumerate(uploaded_files):
            try:
                ...
                dados_temp.append({
                    ...
                    "Cultura": tipo_plantio, "Tecnico": nome_tecnico, "_img_obj": img_com_caixas
                })
                progresso.progress((i + 1) / len(uploaded_files))
            except: continue
        st.session_state.dados_analise = pd.DataFrame(dados_temp)
...
if st.session_state.dados_analise is not None:
    ...
    with col_mapa:
        ...
        if not df_geo.empty:
            ...
            for _, row in df_geo.iterrows():
                ...
                folium.CircleMarker(location=[row['Latitude'], row['Longitude']], radius=10, color=cor, fill=True, popup=f"{row['Pragas']} pragas").add_to(m)
    ...
    with col_intel:
        ...
        fig_candle = go.Figure(data=[go.Candlestick(x=df_top5['Amostra'], open=df_top5['Pragas']*0.9, high=df_top5['Pragas'], low=df_top5['Pragas']*0.7, close=df_top5['Pragas']*0.95)])
        fig_candle.update_layout(height=220, xaxis_rangeslider_visible=False, margin=dict(l=0, r=0, t=0, b=0))
        st.plotly_chart(fig_candle, use_container_width=True)

    # --- SEÇÃO REINSTALADA: RECOMENDAÇÃO TÉCNICA (LOGICA PREMIUM) ---
    st.markdown("---")
    ...
    with rec_col2:
        texto_laudo = (f"O técnico **{nome_tecnico}** realizou o diagnóstico para a cultura de **{tipo_plantio}** na safra **{safra}**. "
                       f"Análise no talhão **{talhao_id}** indica média de **{media_ponto:.1f}** pragas por ponto. ")
        if status_sanitario == "CRÍTICO": texto_laudo += "⚠️ **Ação:** Níveis acima do limite econômico. Intervenção recomendada."
        else: texto_laudo += "👍 **Ação:** Níveis sob controle. Manter monitoramento."
        st.info(texto_laudo)
    ...
    for _, row in df.iterrows():
        ...
        with g2:
            st.markdown(f'<div style="background: white; padding: 20px; border-radius: 15px; border: 1px solid #eee;">'
                        f'<span class="source-tag">{row["Fonte"]}</span><h3>🪲 {row["Pragas"]} Detectadas</h3>'
                        f'<p><b>Amostra:</b> {row["Amostra"]}</p><p><b>Cultura:</b> {row["Cultura"]} | <b>Safra:</b> {row["Safra"]}</p><hr>'
                        f'{"<a href="+row["Maps_Link"]+" target=_blank><button class=loc-btn>📍 LOCALIZAR AGORA</button></a>" if row["Latitude"] != "N/A" else "<i>Sem GPS</i>"}</div>', unsafe_allow_html=True)
        st.markdown("---")
